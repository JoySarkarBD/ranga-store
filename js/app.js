...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    const title = product.title.slice(0, 18);
    const div = document.createElement("div");
    div.classList.add("product");
    div.innerHTML = `<div class="card single-product ms-4 mb-4">
    <img class="product-image mx-auto" src=${image}></img>
      ...
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success w-50">add to cart</button>
      <button type="button" id="details-btn" class="btn btn-danger w-50 ms-2" onclick="singleProduct('${product.id}')">Details</button></div>
      </div>
      `;
...
const singleProduct = (id) => {
  const url = `https://fakestoreapi.com/products/${id}`
        fetch(url)
            .then(res=>res.json())
            .then(json=>singleProductDetail(json))
}
...
const singleProductDetail = product => {
  ...
  <div class="container bg-light mb-4 p-4 border rounded">
  <div class="row">
    <div class="col-3">
    ...
    <dl class="row">
    <dd>
    <h2 class="text-danger display-6">${product.title}</h2>
    ...
    <h4>
    <span class="display-6">Price:<span> ${product.price}
    </h4>
    ...
    <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success mx-auto">add to cart</button>
  </dl>
  `
  // scroll to top at description part
  window.scrollTo(0, 0);
}
